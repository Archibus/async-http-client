...
public class AsyncHttpClientTypesafeConfigTest {
    ...
    public void testThreadPoolName() {
        test(AsyncHttpClientTypesafeConfig::getThreadPoolName, "threadPoolName", "MyHttpClient",
            "AsyncHttpClient");
    }
    ...
    public void testMaxConnectionPerHost() {
        test(AsyncHttpClientTypesafeConfig::getMaxConnectionsPerHost, "maxConnectionsPerHost", 100,
            -1);
    }
    ...
    public void testPooledConnectionIdleTimeout() {
        test(AsyncHttpClientTypesafeConfig::getPooledConnectionIdleTimeout,
            "pooledConnectionIdleTimeout", 200, 6 * 10000);
    }

    public void testReadTimeout() {
        test(AsyncHttpClientTypesafeConfig::getReadTimeout, "readTimeout", 180, 60 * 1000);
    }
    ...
    public void testCompressionEnforced() {
        test(AsyncHttpClientTypesafeConfig::isCompressionEnforced, "compressionEnforced", true,
            false);
    }
    ...
    public void testDisableUrlEncodingForBoundRequests() {
        test(AsyncHttpClientTypesafeConfig::isDisableUrlEncodingForBoundRequests,
            "disableUrlEncodingForBoundRequests", true, false);
    }

    public void testUseInsecureTrustManager() {
        test(AsyncHttpClientTypesafeConfig::isUseInsecureTrustManager, "useInsecureTrustManager",
            true, false);
    }

    public void testEnabledProtocols() {
        test(AsyncHttpClientTypesafeConfig::getEnabledProtocols, "enabledProtocols",
            new String[] { "TLSv1.2", "TLSv1.1" }, new String[] { "TLSv1.2", "TLSv1.1", "TLSv1" },
            Optional.of(obj -> ConfigValueFactory.fromIterable(Arrays.asList(obj))));
    }

    private <T> void test(final Function<AsyncHttpClientTypesafeConfig, T> func,
            final String configKey, final T value, final T defaultValue) {
        test(func, configKey, value, defaultValue, Optional.empty());
    }

    private <T> void test(final Function<AsyncHttpClientTypesafeConfig, T> func,
            final String configKey, final T value, final T defaultValue,
            final Optional<Function<T, ConfigValue>> toConfigValue) {
        final AsyncHttpClientTypesafeConfig defaultConfig =
                new AsyncHttpClientTypesafeConfig(ConfigFactory.empty());
        Assert.assertEquals(func.apply(defaultConfig), defaultValue);

        final AsyncHttpClientTypesafeConfig config =
                new AsyncHttpClientTypesafeConfig(ConfigFactory.empty().withValue(configKey,
                    toConfigValue.orElse(ConfigValueFactory::fromAnyRef).apply(value)));
